...
import { type FocusEvent, type FormEvent, type KeyboardEvent, type MouseEvent, type MutableRefObject, type ReactNode, useRef } from 'react';
...
const DEFAULT_PERIODIC_TABLE_TOGGLE_ICON = (
  <Keyboard aria-hidden="true" focusable="false" size={18} strokeWidth={2} />
);
...
export const MaterialsInputBox = ({
  ...
  const inputOverlayAnchorRef = useRef<HTMLDivElement>(null);
  const inputControl = (
    <div ref={inputOverlayAnchorRef} className="ms-control ms-is-expanded">
      ...
      {autocompleteFormulaUrl ? (
        <FormulaAutocomplete
          anchorRef={inputOverlayAnchorRef}
          value={inputValue}
          inputType={inputType}
